...
##++++++++++++++++++++++++++++++Modeled Climatology +++++++++++++++++++++++++##
## 1. read in your data
data = data.table::fread("LINK TO HISTORICAL SIMULATED CLIMTOLOGY DATA")
data = data[data$dates > as.Date("1979-01-01") & data$dates < as.Date("2005-12-31"), ] ## sub out the time frame
...
data.tw.sum = data.j %>% 
  dplyr::group_by(HUC8, Model,twoweeks) %>% ## Change to Two Weeks if doing daily
  dplyr::summarize(SWE_mod = median(SWE_mm, na.rm = TRUE))

write.csv(data.tw.sum, 
          "[LINK TO HISTORICAL SIMULATED CLIMATOLOGY DATA PROCESSED]")
